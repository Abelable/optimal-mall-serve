...
class GiftCommissionController extends Controller
{
    public function sum()
    {
        ...
        $promoterSettledAmount = GiftCommissionService::getInstance()->getPromoterCommissionSum($this->userId(), [2, 3, 4]);
        $managerSettledAmount = GiftCommissionService::getInstance()->getManagerCommissionSum($this->userId(), [2, 3, 4]);
        ...
        if (!is_null($this->user()->promoterInfo) && $this->user()->promoterInfo->level > 1) {
            ...
            $teamSettledCommission = TeamCommissionService::getInstance()->getUserCommission($this->userId(), [2, 3, 4]);
    ...
    public function timeData()
    {
        ...
        $promoterOrderCount = (clone $promoterQuery)->whereIn('status', [1, 2, 3, 4])->distinct('order_id')->count('order_id');
        $promoterSalesVolume = (clone $promoterQuery)->whereIn('status', [1, 2, 3, 4])->sum('goods_price');
        $promoterPendingAmount = (clone $promoterQuery)->where('status', 1)->sum('promoter_commission');
        $promoterSettledAmount = (clone $promoterQuery)->whereIn('status', [2, 3, 4])->sum('promoter_commission');

        $managerQuery = GiftCommissionService::getInstance()->getManagerCommissionQueryByTimeType($this->userId(), $timeType);
        $managerOrderCount = (clone $managerQuery)->whereIn('status', [1, 2, 3, 4])->distinct('order_id')->count('order_id');
        $managerSalesVolume = (clone $managerQuery)->whereIn('status', [1, 2, 3, 4])->sum('goods_price');
        $managerPendingAmount = (clone $managerQuery)->where('status', 1)->sum('manager_commission');
        $managerSettledAmount = (clone $managerQuery)->whereIn('status', [2, 3, 4])->sum('manager_commission');
